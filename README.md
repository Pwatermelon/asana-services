...
#### Чеклист «самостоятельных проверок» Вебмастера

| Пункт | Как закрыть |
| --- | --- |
| Уведомления Вебмастера | В Вебмастере → Настройки → уведомления на email |
| Оповещения от сайта | Опционально; для каталога не обязательны |
| Целевые запросы | Собрать список (напр. «дханурасана», «каталог асан», названия из источников) в Вебмастере / таблице |
| Группы запросов | Сгруппировать в Вебмастере (каталог / асаны / источники) |
| Посадочные страницы | `/asanas`, `/asana/…-page`, `/sources`, `/about` — уже есть |
| Браузеры | Chrome/Edge/Firefox/Safari; тонкие скроллбары под Windows |
| Страницы не в Disallow | Публичные открыты; `/admin`, `/api`, логин — в `robots.txt` |
| Страницы роботам | Sitemap + переобход `/asanas`, `/sources`, `/about` |
| Контент в базе | Уникальные описания асан/источников; после индексации проверить сниппеты |
| Мониторинг важных страниц | В Вебмастере добавить `/asanas`, `/sources`, `/about` |
| Основной адрес | Главное зеркало `https://catalog-asan.ru` |
| Дубли | Canonical + редирект `/` → `/asanas`; служебное в noindex |
| Ценностный контент | Страница «О проекте» + первоисточники |
| Навигация | Шапка + алфавит + поиск + footer |
| УТП | Некоммерческий каталог традиционных школ с сопоставлением названий (см. «О проекте») |
| Доверие | Footer: email, СГТУ, «О проекте»; авторы и VK на `/about` |
| Заголовки и описания | Динамический SEO на публичных страницах |
| Микроразметка | JSON-LD в `index.html` + на страницах SPA |
| Быстрые ссылки | Чёткая структура разделов; SearchAction; после индексации Яндекс может предложить sitelinks |
| Тематические блоки | Для образовательного/справочного контента — по желанию в партнёрских программах Яндекса |
| Партнёрские обогащённые ответы | Отдельная заявка в Яндексе (не код сайта) |

**Яндекс Бизнес** для некоммерческого справочника не обязателен.
